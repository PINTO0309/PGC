#!/usr/bin/env python3
"""Estimate feature importance for pgc_l_32x32.onnx by ablating intermediate tensors."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default="pgc_l_32x32.onnx", help="Path to ONNX model.")
    ...
    parser.add_argument("--input-size", type=int, default=32, help="Input resolution expected by the model.")
    parser.add_argument(
        "--mean",
        type=float,
        nargs=3,
    ...
    default_heatmap_script = Path(__file__).with_name("10_visualize_pgc_heatmaps.py")
    parser.add_argument(
        "--heatmap-script",
        default=str(default_heatmap_script),
        help="Path to 10_visualize_pgc_heatmaps.py used for auto-rendering top-K heatmaps.",
    )
...
def _load_image(path: Path, input_size: int) -> np.ndarray:
