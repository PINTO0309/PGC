#!/usr/bin/env python3
"""Export Sigmoid->Mul feature maps from pgc_l_32x32.onnx as heatmap PNG images."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run pgc_l_32x32.onnx, capture Conv->Sigmoid->Mul feature maps, and save them as heatmap PNGs."
        )
    )
    parser.add_argument("--model", default="pgc_l_32x32.onnx", help="Path to the ONNX model to inspect.")
    ...
    parser.add_argument("--input-size", type=int, default=32, help="Input resolution (model expects 32).")
    parser.add_argument(
        "--mean",
        type=float,
        nargs=3,
...
def _load_image(path: Path, input_size: int) -> tuple[np.ndarray, np.ndarray]:
