...
class HandAnalysisTracker:
    """Collect statistics and artifacts for hand-only analysis."""

    def __init__(self, output_root: Path) -> None:
        self.output_root = Path(output_root)
        self.output_root.mkdir(parents=True, exist_ok=True)
        self.total_images = 0
        self.images_with_hand = 0
    ...
    @property
    def images_without_hand(self) -> int:
        return max(0, self.total_images - self.images_with_hand)

    def register_image(self, detection_count: int) -> None:
        self.total_images += 1
        if detection_count > 0:
            self.images_with_hand += 1
    ...
    def write_histograms(self) -> None:
        for label_name in sorted(set(self.heights_by_label.keys()) | set(self.widths_by_label.keys())):
            ...
            if not heights and not widths:
                continue
            output_path = self.output_root / f'{label_name}_hand_size_hist.png'
            save_hand_histogram(label_name=label_name, heights=heights, widths=widths, output_path=output_path)

    def print_summary(self) -> None:
        print(Color.GREEN('Hand-only detection summary'))
        print(f'  Total images: {self.total_images}')
        print(f'  Images with detection: {self.images_with_hand}')
        print(f'  Images without detection: {self.images_without_hand}')
        print(f'  Images with >=2 detections: {self.over_detected}')
...
def save_hand_histogram(label_name: str, heights: List[int], widths: List[int], output_path: Path) -> None:
    ...
    fig.suptitle(f'{label_name.capitalize()} hand crop size distribution')
...
def load_label_from_json(image_path: Path) -> Optional[Tuple[str, int]]:
    ...
    try:
        ...
        return None
    label_value = (meta.get('label') or '').strip().lower()
    if label_value in {'closed_hands', 'not_pointing'}:
        return 'not_pointing', 0
    if label_value in {'open_hands', 'pointing'}:
        return 'pointing', 1
    return None
...
def resolve_video_label(video_path: Path) -> Optional[Tuple[str, int]]:
    ...
    if 'not_pointing' in stem or 'closed' in stem:
        return 'not_pointing', 0
    if 'pointing' in stem or 'open' in stem:
        return 'pointing', 1
    return None


def infer_label_from_parts(parts: Iterable[str]) -> Optional[Tuple[str, int]]:
    for part in parts:
        lowered = part.lower()
        if 'not_pointing' in lowered:
            return 'not_pointing', 0
        if 'pointing' in lowered:
            return 'pointing', 1
    return None


def infer_label_from_path(path: Path) -> Optional[Tuple[str, int]]:
    parts = [path.stem] + [ancestor.name for ancestor in path.parents]
    return infer_label_from_parts(parts)


def infer_label_from_directory(path: Path) -> Optional[Tuple[str, int]]:
    return infer_label_from_parts([path.name])


def crop_hand_region(image: np.ndarray, box: Box) -> Optional[np.ndarray]:
    h, w = image.shape[:2]
...
def run_hand_analysis(
    *,
    model: DEIMv2,
    image_dirs: List[Tuple[Path, Optional[Tuple[str, int]]]],
    ...
    for images_dir, label_hint in image_dirs:
        ...
        process_images_dir_for_hand_analysis(
            model=model,
            ...
            disable_left_and_right_hand_identification_mode=disable_left_and_right_hand_identification_mode,
            disable_headpose_identification_mode=disable_headpose_identification_mode,
            label_hint=label_hint or infer_label_from_directory(images_dir),
        )
        processed_any = True
    ...
    for video_path in video_paths:
        ...
        process_video_for_hand_analysis(
            model=model,
    ...
    if not processed_any:
        print(Color.RED('ERROR: No valid sources were processed for hand analysis.'))
        return
...
def process_images_dir_for_hand_analysis(
    *,
    model: DEIMv2,
    images_dir: Path,
    tracker: HandAnalysisTracker,
    crop_indexer: CropIndexer,
    disable_generation_identification_mode: bool,
    disable_gender_identification_mode: bool,
    disable_left_and_right_hand_identification_mode: bool,
    disable_headpose_identification_mode: bool,
    label_hint: Optional[Tuple[str, int]] = None,
) -> None:
    ...
    for idx, image_path_str in enumerate(image_paths):
        ...
        if label_info is None:
            label_info = infer_label_from_path(image_path)
        if label_info is None:
            label_info = label_hint
        if label_info is None:
...
def process_video_for_hand_analysis(
    *,
    model: DEIMv2,
    video_path: Path,
    tracker: HandAnalysisTracker,
...
def main():
    ...
    parser.add_argument(
        ...
        '--hand_analysis',
        action='store_true',
        help='Enable hand-only analysis workflow (cropping, histograms, annotation export).',
    )
    parser.add_argument(
        '-drc',
    ...
    hand_analysis_enabled: bool = args.hand_analysis
    if not hand_analysis_enabled and video is None and images_dir is None:
        parser.error('Either --video or --images_dir must be specified unless --hand_analysis is enabled.')
    ...
    if hand_analysis_enabled:
        image_dirs_for_analysis: List[Tuple[Path, Optional[Tuple[str, int]]]] = []
        video_paths_for_analysis: List[Path] = []

        if images_dir is not None:
            img_dir_path = Path(images_dir)
            image_dirs_for_analysis.append((img_dir_path, infer_label_from_directory(img_dir_path)))

        if video is not None:
            if is_parsable_to_int(video):
                print(Color.YELLOW('Skipping camera index for hand analysis; only file paths are supported.'))
            else:
        ...
        if not image_dirs_for_analysis and not video_paths_for_analysis:
            ...
            image_dirs_for_analysis.extend([
                (dataset_root / 'pointing', ('pointing', 1)),
                (dataset_root / 'not_pointing', ('not_pointing', 0)),
            ])
            ...
            if real_data_dir.exists():
                ...
                matched_paths = [
                    candidate for candidate in real_data_dir.rglob('*.mp4')
                    if candidate.is_file()
                ]
                default_videos.extend(sorted(matched_paths, key=_video_sort_key))
            if not default_videos:
                default_videos = [
                    real_data_dir / 'pointing.mp4',
                    real_data_dir / 'not_pointing.mp4',
                ]
            video_paths_for_analysis.extend(default_videos)

        # Deduplicate while preserving order
        seen_dirs = set()
        unique_image_dirs: List[Tuple[Path, Optional[Tuple[str, int]]]] = []
        for candidate, label_hint in image_dirs_for_analysis:
            resolved = candidate
            key = resolved.resolve()
            if key in seen_dirs:
                continue
            unique_image_dirs.append((resolved, label_hint))
            seen_dirs.add(key)
