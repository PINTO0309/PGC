...
LABEL_MAP = {0: "not_pointing", 1: "pointing"}
...
def _load_deim_module() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("pgc._deimv2_detector", _DEIM_MODULE_PATH)
                ...
                results.append(
                    {
                        ...
                        "prob_pointing": float(probs[idx].detach().cpu().item()),
                    }
                )
    return results
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"pgc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "pgc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"pgc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "pgc_best_", 10)
    ...
    try:
        with torch.no_grad():
            while True:
                ...
                logits = model(tensor)
                prob_pointing = torch.sigmoid(logits)[0].item()

                label = LABEL_MAP[int(prob_pointing >= 0.5)]
                color = (0, 200, 0) if label == "pointing" else (50, 50, 255)
                cv2.putText(
                    frame,
                    f"Prob pointing: {prob_pointing:.2%}",
                    (12, 28),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    color,
...
def run_webcam_inference_onnx(
    onnx_model: Path,
    camera_index: int = 0,
    window_name: str = "PGC Webcam (ONNX)",
    mirror: bool = False,
    ...
    try:
        while True:
            ...
            prob_pointing = float(outputs[0].flatten()[0])
...
def export_to_onnx(
    ...
    torch.onnx.export(
        ...
        output_path,
        input_names=["images"],
        output_names=["prob_pointing"],
        dynamic_axes={"images": {0: "batch"}, "prob_pointing": {0: "batch"}},
        opset_version=opset,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PGC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the PGC binary classifier.")
    ...
    webcam_parser.add_argument("--window_name", type=str, default="PGC Webcam")
    ...
    webcam_onnx_parser.add_argument("--model", type=Path, required=True, help="Path to the exported PGC ONNX model.")
    webcam_onnx_parser.add_argument("--camera_index", type=int, default=0, help="OpenCV camera index (default: 0).")
    webcam_onnx_parser.add_argument("--window_name", type=str, default="PGC Webcam (ONNX)")
