...
class Sample:
    """Lightweight container describing a single cropped pointing frame."""
...
POSITIVE_LABELS = {"open", "pointing"}
NEGATIVE_LABELS = {"closed", "not_pointing"}
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "pointing" if label == 1 else "not_pointing"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'pointing').",
            split,
            total,
...
class PGCDataset(Dataset):
    """Torch dataset that reads pointings crops from disk or embedded bytes."""
